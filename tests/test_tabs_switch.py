...
def test_switch_to_sauce_tab(driver, wait):
    ...
    assert driver.find_element(*SL.ACTIVE_SAUCE_TAB).is_displayed()

def test_switch_to_filling_tab(driver, wait):
    driver.get(MAIN_URL)
    wait.until(EC.element_to_be_clickable(SL.FILLING_TAB)).click()
    wait.until(EC.visibility_of_element_located(SL.ACTIVE_FILLING_TAB))
    assert driver.find_element(*SL.ACTIVE_FILLING_TAB).is_displayed()
...
def test_switch_to_bun_tab(driver, wait):
    ...
    wait.until(EC.visibility_of_element_located(SL.ACTIVE_BUN_TAB))
    assert driver.find_element(*SL.ACTIVE_BUN_TAB).is_displayed()
